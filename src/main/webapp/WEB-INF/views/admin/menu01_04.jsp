										...
										<c:set var="num" value="${pageMaker.totalCount - ((pageMaker.cri.page -1) *10)}"></c:set>
									        <c:forEach var="item" items="${list}">
												<tr class="cont">
													...
													<td><a class="t_link" href="${pageContext.request.contextPath}/admin/menu01_04update${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}"><p class="title"><b>${item.clinic_type}</b> ${item.title}</p></a></td>
													<td>${item.writer}</td>
													<td>${item.regdate}</td>
													<td></td>
													<td>${item.cnt}</td>
